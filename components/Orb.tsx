...
const Orb: React.FC<OrbProps> = ({ state, mood }) => {
  ...
  useEffect(() => {
    if (state === AppState.IDLE || state === AppState.STANDBY || state === AppState.OFFLINE) {
  ...
  const getColors = () => {
    ...
    if (state === AppState.OFFLINE) return 'from-slate-700 via-slate-800 to-slate-900 grayscale opacity-60';
  ...
  const renderFace = () => {
    ...
    if (state === AppState.SLEEP || state === AppState.OFFLINE) {
      browL = "M25 40 Q35 42 45 40";
      browR = "M55 40 Q65 42 75 40";
      mouthPath = "M45 78 Q50 80 55 78";
      pupilSize = state === AppState.OFFLINE ? 2 : 0;
