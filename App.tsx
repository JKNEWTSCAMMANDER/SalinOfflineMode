...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const handleOnline = () => {
      setIsOnline(true);
      if (state === AppState.OFFLINE) setState(AppState.IDLE);
    };
    const handleOffline = () => {
      setIsOnline(false);
    ...
    if (!navigator.onLine) setState(AppState.OFFLINE);
  ...
  const stopConversation = useCallback((forceSleep = false) => {
    ...
    if (forceSleep) {
      ...
      setTimeout(() => {
        setState(isOnline ? AppState.IDLE : AppState.OFFLINE);
  ...
  const startConversation = async () => {
    ...
    try {
      ...
      const sessionPromise = ai.live.connect({
        ...
        callbacks: {
          ...
          onerror: (err) => {
            ...
            setErrorMessage('Connection failed. Please check your internet and API key.');
            setState(AppState.ERROR);
          },
          onclose: () => {
            if (state !== AppState.SLEEP) setState(isOnline ? AppState.IDLE : AppState.OFFLINE);
          }
        }
      });
      ...
      setErrorMessage(`Failed to start: ${error.message || 'Check microphone & internet'}`);
      setState(AppState.ERROR);
    }
  };
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden">
        <div className="flex-[2] flex items-center justify-center relative">
          ...
          {state === AppState.OFFLINE && (
            <div className="absolute top-4 px-3 py-1 bg-amber-500/10 border border-amber-500/20 rounded-full backdrop-blur-sm">
                <span className="text-[8px] text-amber-500 font-bold uppercase tracking-widest">No Internet Connection</span>
             </div>
          )}
        ...
        <div className="absolute bottom-0 left-0 right-0 px-8 py-6 flex items-center justify-between bg-gradient-to-t from-slate-950 via-slate-950 to-transparent z-30">
          <button
            ...
            disabled={!isOnline || state === AppState.CONNECTING || state === AppState.LISTENING || state === AppState.SPEAKING}
          ...
          <div className="relative transform -translate-y-2">
            {state === AppState.IDLE || state === AppState.STANDBY || state === AppState.SLEEP || state === AppState.ERROR || state === AppState.OFFLINE ? (
              <button
                onClick={() => startConversation()}
                disabled={!isOnline}
                className={`group flex items-center justify-center w-16 h-16 bg-gradient-to-br from-blue-600 to-indigo-700 rounded-full transition-all active:scale-90 shadow-2xl shadow-blue-500/20 disabled:grayscale disabled:opacity-50`}
              >
